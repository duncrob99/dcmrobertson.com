...
export async function hashPassword(
	password: string
): Promise<{ exported_key: string; salt: string }> {
	const salt_array = new Uint8Array(18);
	...
	const exported_key_array = await crypto.subtle.exportKey('raw', passwordKey);
	const exported_key = btoa(String.fromCharCode(...new Uint8Array(exported_key_array)));
	const salt = btoa(String.fromCharCode(...salt_array));
...
export async function checkUserAuth(cookies: Cookies): Promise<User | undefined> {
	...
	const auth_token_array = fromBase64(auth_token);

	const session_token_query = await client.execute({
	...
	const session_token = session_token_query.rows[0];
	const user_email = session_token.user_email;
	const token_expiry = session_token.expiry;

	if (
		!user_email ||
		!token_expiry ||
		!(typeof user_email === 'string') ||
		!(typeof token_expiry === 'number')
	) {
		return;
	}
