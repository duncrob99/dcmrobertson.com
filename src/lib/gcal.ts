...
import { TimeRange, Time, AppointmentState, Day, type Availability } from "$lib/types";
...
async function getCalendarEvents(rangeStart: DateTime, rangeEnd: DateTime): Promise<{busy_events: Event[], tutoring_events: Event[]}> {
	const busy_events = (await Promise.all(busy_calendars.map(cal => getEvents(cal, rangeStart, rangeEnd)))).flat();
	const tutoring_events = await getEvents(tutoring_calendar, rangeStart, rangeEnd);
	return {busy_events, tutoring_events};
}

function calculateAvailability(busy_events: Event[], tutoring_events: Event[], rangeStart: DateTime, rangeEnd: DateTime): Availability[] {
